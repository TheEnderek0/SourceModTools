    ...
    def __init__(self, use_colors = True):
        ...
        if use_colors:
            self.formats = {
                ...
                logging.WARN: logging.Formatter(Style.NORMAL + Fore.LIGHTYELLOW_EX + "[W] %(name)s: %(message)s " + Style.RESET_ALL),
                logging.ERROR: logging.Formatter(Style.NORMAL + Fore.RED + "[E] %(name)s: %(message)s " + Style.RESET_ALL),
...
def SetupLogger(log_file: Path, debug_mode = False, create_files = True):
    ...
    if log_file.exists() and create_files:
    ...
    logging.basicConfig(
        handlers = [file_handler, console_handler] if create_files else [console_handler],
